...
return {
  on_attach = function(client)
    -- Biome handles formatting; prevent vtsls from competing.
    client.server_capabilities.documentFormattingProvider = false
    client.server_capabilities.documentRangeFormattingProvider = false
  end,
  root_dir = function(bufnr, on_dir)
    -- tsc (nvim-lspconfig's TS7+ lsp) handles TS7 and above; vtsls only
    -- activates below that, so the nearest installed `typescript` package
    -- doubles as both the root marker and the version gate.
